...
import { mockDailyReportRows, DailyReportRow } from '../data/mockDailyReportData';
...
export const DailyAttendanceReportView: React.FC<Props> = ({ onExport, onSendEmail }) => {
  // Filters state
  ...
  const filteredRows = useMemo(() => {
    if (!searchTerm.trim()) return mockDailyReportRows;
    const term = searchTerm.toLowerCase();
    return mockDailyReportRows.filter(
      (r) =>
        r.factoryDept.toLowerCase().includes(term) ||
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col space-y-3">
      {/* Top Header & Filters Section (Shrink-0) */}
      <div className="shrink-0 space-y-3">
        {/* Top Header Row matching Screenshot 9 */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {/* Export & Email Buttons (Green as in Screenshot 9) */}
        ...
        {/* Category Toggle Checkboxes & Show/Hide Absence & Trip Button matching Screenshot 9 */}
      ...
      {/* Main Wide Attendance Report Table Container (Flex-1, Docked to bottom) */}
      <div className="flex-1 min-h-0 flex flex-col rounded-xl border border-slate-200/80 overflow-hidden">
        {/* Table Scrollable Body (100% Remaining Height) */}
        <div className="flex-1 overflow-auto relative w-full h-full">
          <table className="w-full text-[13px] text-center border-separate border-spacing-0">
            {/* Table Header with 2 header rows */}
            <thead className="bg-[#f1f5f9] sticky top-0 z-20 shadow-xs text-slate-700 font-bold">
              {/* Row 1: Header Group Titles */}
              <tr className="bg-[#f1f5f9]">
                <th
                  rowSpan={2}
                  className="px-3 py-2.5 bg-[#f1f5f9] min-w-[120px] sticky left-0 z-30 border-r border-b border-slate-300"
                >
                  廠部
                </th>
                <th
                  rowSpan={2}
                  className="px-3 py-2.5 bg-[#f1f5f9] min-w-[150px] sticky left-[120px] z-30 border-r border-b border-slate-300"
                >
                  部門名稱
                </th>
                <th
                  rowSpan={2}
                  className="px-3 py-2.5 bg-[#f1f5f9] min-w-[80px] sticky left-[270px] z-30 border-r border-b border-slate-300"
                >
                  昨日在職
                </th>
              ...
              <tr className="text-[11px] font-semibold text-slate-600 bg-[#f8fafc]">
                ...
                <th className="px-2 py-1.5 min-w-[55px] font-bold text-slate-800 bg-[#e2e8f0] border-r border-b border-slate-300">總人數</th>
                ...
                {/* 缺勤分佈 Sub-columns (Optional - 3 sub-columns: 曠職, 請假(未簽核完成), 請假(已簽核完成)) */}
                {showAbsenceAndTrip && (
                  <>
            ...
            <tbody className="text-slate-700 font-medium bg-white">
              {/* 合計 Row 1 (Solid Amber Background, No Transparency) */}
              <tr className="bg-[#fffbeb] font-bold text-slate-900 hover:bg-[#fef3c7] transition-colors">
                ...
                <td className="px-3 py-2 sticky left-[120px] z-10 bg-[#fffbeb] border-r border-b border-amber-300 shadow-r"></td>
                <td className="px-3 py-2 sticky left-[270px] z-10 bg-[#fffbeb] border-r border-b border-amber-300 shadow-r">
                ...
                <td className="px-2 py-2 font-bold bg-[#fef3c7] border-r border-b border-amber-200">{summaryRow.todayOnDuty.total}</td>
                ...
                <td className="px-2 py-2 font-bold bg-[#fef3c7] border-r border-b border-amber-200">{summaryRow.expectedPresent.total}</td>
                ...
                <td className="px-2 py-2 font-bold bg-[#fef3c7] border-r border-b border-amber-200">{summaryRow.actualPresent.total}</td>
                ...
                {/* 缺勤分佈 合計 (3 sub-columns: 曠職, 請假(未簽核完成), 請假(已簽核完成)) */}
                {showAbsenceAndTrip && (
                  <>
                    <td className="px-2 py-2 font-bold border-r border-b border-amber-200">{summaryRow.absentTruancy}</td>
              ...
              {filteredRows.map((row) => (
                <tr key={row.id} className="hover:bg-[#f8fafc] transition-colors group">
                  <td className="px-3 py-2 sticky left-0 z-10 bg-white group-hover:bg-[#f8fafc] border-r border-b border-slate-200 shadow-r text-left font-medium">
                    {row.factoryDept}
                  </td>
                  <td className="px-3 py-2 sticky left-[120px] z-10 bg-white group-hover:bg-[#f8fafc] border-r border-b border-slate-200 shadow-r text-left">
                    {row.deptName}
                  </td>
                  <td className="px-3 py-2 sticky left-[270px] z-10 bg-white group-hover:bg-[#f8fafc] border-r border-b border-slate-200 shadow-r font-mono">
                    {row.yesterdayOnDuty}
                  </td>
                  ...
                  {/* 缺勤分佈 (3 sub-columns: 曠職, 請假(未簽核完成), 請假(已簽核完成)) */}
                  {showAbsenceAndTrip && (
                    <>
                      <td className="px-2 py-2 font-mono border-r border-b border-slate-200">
                        {row.absentTruancy !== '-' && row.absentTruancy !== 0 ? (
        ...
        {/* Footer Pagination Bar matching Screenshot 9 */}
